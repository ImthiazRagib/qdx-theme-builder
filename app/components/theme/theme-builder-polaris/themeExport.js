...
.product-grid { display: grid; gap: 16px; grid-template-columns: repeat(2, minmax(0, 1fr)); }
@media (min-width: 1024px) { .product-grid { grid-template-columns: repeat(4, minmax(0, 1fr)); } }
.product-grid--2-col { grid-template-columns: repeat(2, minmax(0, 1fr)); }
.product-grid--3-col { grid-template-columns: repeat(2, minmax(0, 1fr)); }
.product-grid--4-col { grid-template-columns: repeat(2, minmax(0, 1fr)); }
@media (min-width: 768px) { .product-grid--3-col { grid-template-columns: repeat(3, minmax(0, 1fr)); } }
@media (min-width: 768px) { .product-grid--4-col { grid-template-columns: repeat(4, minmax(0, 1fr)); } }
.product-card { background: #fff; border: 1px solid #e5e7eb; border-radius: 14px; overflow: hidden; display: flex; flex-direction: column; height: 100%; transition: box-shadow .2s ease, transform .2s ease; }
.product-card:hover { box-shadow: 0 14px 32px rgba(15, 23, 42, .12); transform: translateY(-2px); }
.product-card__media-link { display: block; color: inherit; text-decoration: none; }
.product-card__media { position: relative; background: #f8fafc; aspect-ratio: 1 / 1; overflow: hidden; }
...
.product-card__badge { position: absolute; top: 10px; left: 10px; background: #111827; color: #fff; border-radius: 999px; padding: 3px 10px; font-size: 11px; font-weight: 600; letter-spacing: .01em; }
.product-card__content { padding: 12px 12px 14px; display: flex; flex-direction: column; gap: 6px; }
.product-card__eyebrow { margin: 0; color: #6b7280; font-size: 12px; line-height: 1.4; }
.product-card__title { margin: 0; font-size: 15px; line-height: 1.35; font-weight: 600; color: #111827; }
.product-card__title a { color: inherit; text-decoration: none; }
.product-card__title a:hover { text-decoration: underline; }
.product-card__price-row { display: flex; align-items: center; flex-wrap: wrap; gap: 8px; margin-top: 2px; }
.product-card__price { font-size: 14px; font-weight: 700; color: #111827; }
.product-card__compare { font-size: 13px; color: #9ca3af; }
.product-card__media--placeholder { background: #f8fafc; }
.product-card__image-placeholder { width: 100%; height: 100%; min-height: 170px; background: #e5e7eb radial-gradient(circle at 30% 20%, #f3f4f6, #d1d5db); }
.product-card__title-placeholder { width: 72%; height: 15px; border-radius: 999px; background: #e5e7eb; }
.product-card__price-placeholder { width: 72px; height: 14px; border-radius: 999px; background: #d1d5db; }
.product-card__compare-placeholder { width: 58px; height: 13px; border-radius: 999px; background: #e5e7eb; }
...
const SNIPPET_PRODUCT_CARD = `{% comment %} Product card generated by Theme Creator {% endcomment %}
<article class="product-card">
  ...
  <div class="product-card__content">
    <p class="product-card__eyebrow">{{ product.vendor | default: 'Featured product' }}</p>
...
const SNIPPET_PRODUCT_CARD_PLACEHOLDER = `<article class="product-card product-card--placeholder">
  <div class="product-card__media product-card__media--placeholder">
    <div class="product-card__image-placeholder"></div>
  </div>
  <div class="product-card__content">
    <p class="product-card__eyebrow">Featured product</p>
...
export async function exportThemeAsZip({ homeSections = [], productSections = [], themeColors = {} }) {
  ...
  zip.file('templates/index.json', JSON.stringify(toTemplateData(homeSections), null, 2));
  zip.file('templates/product.json', JSON.stringify(toTemplateData(productSections), null, 2));
